...
internal static class StartGameHack
{
    ...
    private static bool OnStartGame(StartOfRound __instance)
    {
        ...
        {
            if (player.ReportHack(Detection.StartGame,
                    $"Player {player.playerUsername} tried to start the ship even though only the host may start the ship"))
                return false;
        }
        ...
        {
            if (player.ReportHack(Detection.StartGame,
                    $"Player {player.playerUsername} tried to start the ship from too far away"))
                return false;
        }
